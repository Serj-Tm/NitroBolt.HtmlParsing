            ...
            else
            {
                ...
                switch (this.m_resolvedUri.Scheme)
                {
                    ...
                    default:
                        ...
                        if (actual.AbsoluteUri != this.m_resolvedUri.AbsoluteUri)
                        {
...
*/
    ...
    public class SgmlDtd
    {
        ...
        /// <param name="nt">The <see cref="XmlNameTable"/> to use to store atomized strings.</param>
        ...
        private Entity ParseParameterEntity(string term)
        {
            ...
            name = this.m_nameTable.Add(name);
        ...
        private void ParseElementDecl()
        {
            ...
            foreach (string name in names) 
            {
                ...
                atom = this.m_nameTable.Add(name); 
            ...
            if (ch == '(') 
            {
                ...
                while (ch != ')') 
                {
                    ...
                    else 
                    {
                        ...
                        string atom = this.m_nameTable.Add(token);
                        names.Add(atom);
                    }
                    ch = this.m_current.SkipWhitespace();
        ...
        void ParseModel(char cmt, ContentModel cm)
        {
            ...
            while (ch != cmt || cm.CurrentDepth > depth) // the entity must terminate while inside the content model.
            {
                ...
                else
                {
                    ...
                    token = token.ToUpper(CultureInfo.InvariantCulture);
                    token = this.m_nameTable.Add(token);// atomize it.
                    ch = this.m_current.Lastchar;
                    if (ch == '?' || ch == '+' || ch == '*') 
                    {
                        cm.PushGroup();
