...
/*            set
    ...
    internal class Node
    {
        ...
        public Attribute AddAttribute(string name, string value, char quotechar, bool caseInsensitive) {
          //DarkGray обрабатываем вариант вида <input selected value='a'/> 
          if (value == null)
            value = "";
          //DarkGray
            Attribute a;
            ...
            for (int i = 0, n = this.attributes.Count; i < n; i++) {
                ...
                if (caseInsensitive && string.Compare(a.Name, name, true, CultureInfo.InvariantCulture) == 0)
                {
                    return null;
                }
    ...
    public class SgmlReader : XmlReader
    {
        ...
        /// <summary>
        /// Initialises a new instance of the SgmlReader class with an existing <see cref="XmlNameTable"/>
        /// </summary>
        ...
        private void LazyLoadDtd(Uri baseUri)
        {
            if (this.m_dtd == null)
            {
                if (string.IsNullOrEmpty(this.m_syslit))
                {
                    if (this.m_docType != null && StringUtilities.EqualsIgnoreCase(this.m_docType, "html"))
                    {
                        ...
                        if (stm != null)
                        {
                            ...
                            this.m_dtd = SgmlDtd.Parse(baseUri, "HTML", sr, null, this.m_proxy, this.m_nametable);
                        }
                    }
                }
                else
                { 
                    ...
                    this.m_dtd = SgmlDtd.Parse(baseUri, this.m_docType, this.m_pubid, baseUri.AbsoluteUri, this.m_subset, this.m_proxy, this.m_nametable);
                }
            }
        ...
        public override string NamespaceURI
        {
            get
            {
                ...
                if (this.m_state == State.Attr && (string.Compare(this.m_a.Name, "xmlns", StringComparison.InvariantCulture) == 0))
                {
            ...
            while (!foundnode)
            {
                switch (this.m_state)
                {
                    ...
                    case State.Eof:
                        ...
                        break;
                    case State.EndTag:
                        if (this.m_endTag == (object)this.m_node.Name)
                        {
                            Pop(); // we're done!
                            this.m_state = State.Markup;
                            goto case State.Markup;
        ...
        private string ScanName(string terminators)
        {
            ...
            return this.m_nametable.Add(name);
        }
        ...
        private bool ParseEndTag()
        {
            ...
            {
                Node n = (Node)this.m_stack[i];
                if (caseInsensitive && string.Compare(n.Name, name, true, CultureInfo.InvariantCulture) == 0)
                {
        ...
        private void ParseDocType()
        {
            ...
            if (ch != '>')
            {
                ...
                if (ch != '[')
                {
                    ...
                    if (ch == '\"' || ch == '\'')
                    {
                        token = this.m_nametable.Add("SYSTEM");
        ...
        private bool ParsePI()
        {
            ...
            if (name != "xml")
            {
                Push(m_nametable.Add(name), XmlNodeType.ProcessingInstruction, value);
                return true;
            }
        ...
        private string ScanLiteral(StringBuilder sb, char quote)
        {
            ...
            while (ch != Entity.EOF && ch != quote && ch != '>')
            {
                if (ch == '&')
                {
                    // TODO: Is this correct?  Should this be the local string builder instead?  I believe the method is only ever called with m_sb anyway.
                    ExpandEntity(this.m_sb, quote);
                    ch = this.m_current.Lastchar;
                }               
                else
                {
            ...
            while (ch != Entity.EOF)
            {
                if (ch == '<')
                {
                    ...
                    else if (ch == '/')
                    {
                        ...
                        if (ParseEndTag() && this.m_endTag == (object)this.m_node.Name)
                        {
        ...
        private void ValidateContent(Node node)
        {
            ...
            if (this.m_dtd != null)
            {
                ...
                string name = this.m_nametable.Add(node.Name.ToUpper(CultureInfo.InvariantCulture)); // DTD is in upper case
                int i = 0;
                ...
                if (node.DtdType != null) { 
                    ...
                    for (i = top; i > 0; i--)
                    {
                        ...
                        if (f != null)
                        {
                            if ((i == 2) && (f.Name == "BODY")) // NOTE (steveb): never close the BODY tag too early
                                break;
                            else if (f.Name == this.m_dtd.Name)
